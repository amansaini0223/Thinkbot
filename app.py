...
with st.sidebar:
    ...
    search_query = st.text_input(
        "🔍 Search Memory",
        placeholder="Find past messages...",
        help="Filters the chat history in real-time.",
    )
    ...
    with st.expander("⚙️ Core Settings", expanded=True):
        selected_model_name = st.selectbox(
            "LLM Engine", options=list(config.MODELS.keys())
        )
    ...
    with st.expander("🎛️ Output Tuning", expanded=False):
        length = st.select_slider(
            "Length", options=list(config.LENGTH_MODIFIERS.keys()), value="Medium"
        )
        tone = st.select_slider(
            "Tone", options=list(config.TONE_MODIFIERS.keys()), value="Professional"
        )
    ...
    with st.expander("🔌 Tools & Plugins", expanded=False):
        enhance_prompt = st.toggle(
            "✨ Prompt Enhancer", help="Automatically optimizes prompt.", value=True
        )
        web_search = st.toggle(
            "🌐 Web Search (Mock)", help="Simulates internet access."
        )
    ...
    with col1:
        ...
        if not txt_disabled:
            txt_data = export.export_chat_to_text(
                st.session_state.messages, app_name=config.APP_NAME
            )
            st.download_button(
                "TXT",
                data=txt_data,
                file_name="nexora_session.txt",
                mime="text/plain",
    ...
    with col2:
        if not txt_disabled:
            md_data = export.export_chat_to_markdown(
                st.session_state.messages, app_name=config.APP_NAME
            )
            st.download_button(
                "MD",
                data=md_data,
                file_name="nexora_session.md",
                mime="text/markdown",
                use_container_width=True,
            )
        else:
...
if not st.session_state.messages:
    ...
    st.markdown(
        f"<h1 class='main-title'>Welcome to {config.APP_NAME}</h1>",
        unsafe_allow_html=True,
    )
    st.markdown(
        f"<p class='tagline'>INITIALIZING {selected_model_name.upper()} CORE...</p>",
        unsafe_allow_html=True,
    )
...
prompt = st.chat_input("Communicate with Tech_AI...")
...
if prompt:
    if not client:
        st.error(
            "⚠️ System Failure: Groq API Key is missing. Please check your .env configuration."
        )
    else:
        st.session_state.follow_up_suggestions = []
        ...
        if data_upload:
            actual_prompt = (
                f"[Context: User uploaded {data_upload.name}]\n" + actual_prompt
            )
        ...
        if enhance_prompt:
            with st.spinner("✨ Optimizing neural prompt..."):
                actual_prompt = ai_service.enhance_prompt_internally(
                    client, actual_prompt, model=selected_model
                )
        ...
        with st.chat_message("assistant"):
            try:
                ...
                st.session_state.follow_up_suggestions = (
                    ai_service.generate_follow_up_questions(
                        client,
                        st.session_state.messages,
                        response_content,
                        model=selected_model,
...
if st.session_state.follow_up_suggestions:
    st.markdown(
        "<div style='margin-top: 1rem;'><small style='color: #00F0FF;'><b>Suggested logic paths:</b></small></div>",
        unsafe_allow_html=True,
    )
